<div align="center">
  <h1>Hey 👋, I'm Akshit Garg</h1>
  <h3>B.Tech CSE Graduate | Software Engineer | Backend & Full-Stack Developer</h3>

<p>
I enjoy building scalable backend systems, full-stack web applications, and practical software solutions. My interests include backend engineering, authentication systems, distributed applications, databases, and applying machine learning to solve real-world problems. I'm currently seeking Software Engineer opportunities where I can contribute to impactful products while continuously improving my engineering skills.
</p>

---
<div align="center">
<h2>🚀 Tech Stack</h2>

<h3>💻 Languages</h3>
<img src="https://skillicons.dev/icons?i=cpp,python,js,sql" />

<br>

<h3>⚙️ Backend</h3>
<img src="https://skillicons.dev/icons?i=fastapi,flask,nodejs" />

<br>

<h3>🎨 Frontend</h3>
<img src="https://skillicons.dev/icons?i=react,html,css,tailwind" />

<br>

<h3>🗄️ Databases</h3>
<img src="https://skillicons.dev/icons?i=postgres,mysql,firebase,redis" />

<br>

<h3>🛠 Tools</h3>
<img src="https://skillicons.dev/icons?i=git,github,docker,linux,postman" />

<br>

<h3>📚 Libraries</h3>
<img src="https://skillicons.dev/icons?i=sklearn,pandas,numpy" />

</div>

---

## 🏆 My Projects & Work

Here are a few real-world projects I've built — each focused on applying core CS concepts in practical scenarios.

<table>
  <thead>
    <tr>
      <th width="50%">Project</th>
      <th>Tech Stack</th>
      <th>Links</th>
    </tr>
  </thead>
  <tbody>
    <tr>
<td>

<strong>TradeSmart – ML Powered Scrap Trading Platform</strong>

<br><br>

Built a full-stack peer-to-peer marketplace for buying and selling second-hand products using React.js and Flask. Integrated MobileNetV2-based image classification, secure authentication, relational database design using SQLAlchemy, and deployed the complete application on Vercel and Render.

<br><br>

<em>Tech:</em> React.js • Flask • PostgreSQL • SQLAlchemy • MobileNetV2

</td>

<td align="center">

<img src="https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react"/>
<img src="https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask"/>
<img src="https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql"/>
<img src="https://img.shields.io/badge/SQLAlchemy-D71F00?style=for-the-badge"/>

</td>

<td align="center">

<a href="YOUR_REPO">🔗 GitHub</a>

</td>

</tr>   <tr>
<td>

<strong>RoleGuard – Enterprise IAM Backend</strong>

<br><br>

Designed and developed a secure Identity and Access Management backend using FastAPI. Implemented JWT authentication, Role-Based Access Control (RBAC), Redis caching, PostgreSQL integration, and REST APIs for secure enterprise user management.

<br><br>

<em>Tech:</em> FastAPI • PostgreSQL • Redis • JWT

</td>

<td align="center">

<img src="https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi"/>
<img src="https://img.shields.io/badge/PostgreSQL-336791?style=for-the-badge&logo=postgresql"/>
<img src="https://img.shields.io/badge/Redis-DC382D?style=for-the-badge&logo=redis"/>
<img src="https://img.shields.io/badge/JWT-black?style=for-the-badge"/>

</td>

<td align="center">

<a href="YOUR_REPO">🔗 GitHub</a>

</td>

</tr>   <tr>
<td>

<strong>SyncBoard – Real-Time Collaborative Workspace</strong>

<br><br>

Built a collaborative workspace backend supporting real-time messaging, task synchronization, and multi-user collaboration using FastAPI and WebSockets. Integrated Redis for active session management and PostgreSQL for persistent project data.

<br><br>

<em>Tech:</em> FastAPI • React • PostgreSQL • Redis • WebSockets

</td>

<td align="center">

<img src="https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi"/>
<img src="https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react"/>
<img src="https://img.shields.io/badge/PostgreSQL-336791?style=for-the-badge&logo=postgresql"/>
<img src="https://img.shields.io/badge/Redis-DC382D?style=for-the-badge&logo=redis"/>

</td>

<td align="center">

<a href="YOUR_REPO">🔗 GitHub</a>

</td>

</tr>
</tbody>
</table>

---

## 🛠️ Problem Solving & Consistency

<div align="center">
  <img src="https://leetcode-badge-showcase.vercel.app/api?username=akshit12_g&theme=dark" alt="LeetCode Badge" />
</div>

> **2026 Milestone:** Completed the **January** & **February** Daily Challenge streaks.
> **Current Focus:** Mastering DSA patterns through daily discipline.

---

## 📈 GitHub Statistics

<div align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=Akshitgarg1&show_icons=true&theme=github_dark&hide_border=true"/>

<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Akshitgarg1&layout=compact&theme=github_dark&hide_border=true"/>

<br><br>

<img src="https://streak-stats.demolab.com?user=Akshitgarg1&theme=github-dark-blue&hide_border=true"/>

<br><br>

<img src="https://github-readme-activity-graph.vercel.app/graph?username=Akshitgarg1&theme=github-dark"/>

</div>

---


## 🎓 Education

<table>
  <thead>
    <tr>
      <th width="70%">Institution</th>
      <th>Duration</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>
        <strong>Bachelor of Technology (B.Tech) in Computer Science & Engineering</strong>
        <br/>
        Graphic Era Hill Unieversity, Dehradun
        <br/>
        Key Courses: Data Structures, Algorithms, Operating Systems, DBMS, Computer Networks, Compiler Design, Machine Learning
        <br/>
        CGPA : 8.53 / 10
      </td>
      <td align="center">2022 – 2026</td>
    </tr>
    <tr>
      <td>
        <strong> CBSE (Class XII) – Science (PCM)</strong>
        <br/>
        Greenway Modern Sr. Sec. School, Roorkee
        <br/>
        Percentage: 78.40%
      </td>
      <td align="center">2021 – 2022</td>
    </tr>
    <tr>
      <td>
        <strong>CBSE (Class X)</strong>
        <br/>
        Greenway Modern Sr. Sec. School, Roorkee
        <br/>
        Percentage: 90.60%
      </td>
      <td align="center">2019 – 2020</td>
    </tr>
  </tbody>
</table>

---


## 📫 Let's Connect!

You can find me on:

<p align="center">
  <a href="https://www.linkedin.com/in/akshitgarg26/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white">
  </a>
  <a href="mailto:garg28966@gmail.com" target="_blank">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white">
  </a>
  <a href="https://leetcode.com/u/akshit12_g/" target="_blank">
    <img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=white">
  </a>
  <a href="https://github.com/Akshitgarg1" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white">
  </a>
</p>

---

<p align="center">

<img src="https://komarev.com/ghpvc/?username=Akshitgarg1&label=Profile%20Views&color=0e75b6&style=flat"/>

</p>
